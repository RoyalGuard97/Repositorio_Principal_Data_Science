DROP DATABASE henry; #Eliminas una Base de Datos
CREATE DATABASE henry; #Creas una base de datos 

USE henry; #Usas una base de datos

DROP TABLE carrera; #Dropeas una tabla
CREATE TABLE carrera ( #Creas una tabla llamada carrera
	idCarrera INT NOT NULL AUTO_INCREMENT, #Dato: IdCarrera, este entero sera no nulo y se autoincrementara
	nombre VARCHAR (20) NOT NULL, #Dato: nombre es una cadena de 20 caracteres que no admite nulos
	PRIMARY KEY (idCarrera) #La llave primaria, dato único será idCarrera
);

#Prueba de visualizacion
SELECT * FROM carrera; #Funcionó correctamente. 

DROP TABLE instructor; 
CREATE TABLE instructor ( #Creas la tabla llamada instructor
	idInstructor INT NOT NULL AUTO_INCREMENT , #Un id autoincremental
	cedulaIdentidad VARCHAR(25) NOT NULL, #un string de 25 caracteres que no admite nulos
	nombre VARCHAR (45) NOT NULL, #un string de 45 caracteres que no admite nulos
	apellido VARCHAR (45) NOT NULL, #Un string de 45 caracteres que no admite nulos
	fechaNacimiento DATE NOT NULL, #un tipo de dato fecha que no admite nulo
	fechaIncorporacion DATE, #Un tipo de dato fecha, admite nulos
	PRIMARY KEY (idInstructor) #Mi llave primaria de la tabla que no se repite
);
...
CREATE TABLE cohorte (
	idCohorte INT NOT NULL AUTO_INCREMENT, #Dato incremental
	codigo VARCHAR (45) NOT NULL, #String
	idCarrera INT NOT NULL, #entero que no puede ser nulo
    idInstructor INT, #Entero que admite nulo
	fechaInicio DATE, #tipo de dato fecha que admite nulo
	fechaFinalizacion DATE, #tipo de dato fecha admite nulo
	PRIMARY KEY (idCohorte), #llave primaria que es el id
	FOREIGN KEY (idCarrera) REFERENCES carrera(idCarrera), #llave foranea que se toma de carrera
	FOREIGN KEY (idInstructor) REFERENCES instructor(idInstructor) #llave foranea que se toma de instructor
);
...
#La clave es crear un camino entre los datos
select DISTINCT concat(i.nombre," ",i.apellido) as nombre_completo
from instructor i
join cohorte c on (i.idInstructor = c.idInstructor)
...
select distinct count(*) as cantidad_por_carrera, ca.nombre
from alumno a
join cohorte c on (a.idCohorte = c.idCohorte)
join carrera ca on (c.idCarrera = ca.idCarrera)
GROUP BY ca.nombre;

select idCohorte,count(*) as alumnos_por_cohorte
from alumno
GROUP BY idCohorte;
...
select * from instructor where idInstructor in 
